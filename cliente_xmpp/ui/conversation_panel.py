...
class ConversationPanel(wx.Panel):
    ...
    def _format_poll(poll: Poll) -> str:
        votes = poll.option_votes
        ranked = sorted(
            enumerate(poll.options),
            key=lambda item: (
                -(votes[item[0]] if item[0] < len(votes) else 0),
                item[0],
            ),
        )
        lines = [f"🗳 {poll.title}"]
        for index, option in ranked:
            count = votes[index] if index < len(votes) else 0
            noun = "voto" if count == 1 else "votos"
            lines.append(f"☐ {option}: {count} {noun}")
        return "\n".join(lines)
